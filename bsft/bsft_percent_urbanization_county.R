...
studyarea_nlcd_proj <- project(studyarea_nlcd, crs(nc))
second_layer <- studyarea_nlcd_proj[[2]]
second_layer <- reclassify(second_layer)

plot(second_layer)
second_layer$color <- ifelse(second_layer$your_attribute %in% c(11:12), "lightblue",
                   ifelse(second_layer$your_attribute %in% c(21:24), "darkgrey",
                          ifelse(second_layer$your_attribute %in% c(41:43), "lightgreen", "white")))
